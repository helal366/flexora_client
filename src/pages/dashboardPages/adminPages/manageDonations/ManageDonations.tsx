...
const ManageDonations = () => {
    ...
    const fetchDonations = async ():Promise<IDonation[]> => {
        const res = await axiosSecure.get('/donations');
        return res.data;
    };
    const updateDonationStatus = async ({ id, status }:IUpdatePayload) => {
    ...
    return (
      <div className="overflow-x-auto p-4 bg-white rounded shadow">
        ...
        <table className="table w-full table-auto border-collapse border border-gray-300">
          <thead>
            <tr className="bg-teal-100">
              ...
              <th className="border border-gray-300 p-2 whitespace-break-spaces">
          ...
          <tbody>
            ...
            {donations.map((donation) => (
              <tr key={donation._id} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 p-2">
                  {" "}
                  {donation.posted_at?.split("T")[0]}
                </td>
                <td className="border border-gray-300 p-2">
                  {" "}
                  {donation.posted_at?.split("T")[1].slice(0, 8)}
                </td>
                <td className="border border-gray-300 p-2">
        ...
        {mutation.isPending && (
          <p className="mt-2 text-blue-600">Updating status...</p>
        )}
        {mutation.isError && (
          <p className="mt-2 text-red-600">
            Error updating status: {mutation.error.message}
          </p>
        )}
      </div>
    );
