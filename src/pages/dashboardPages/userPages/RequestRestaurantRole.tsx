...
import {
  RestaurantRequestFormPayload,
  RestaurantRequestPatchData,
} from "../../../types/users";
import { RestaurantRolePatchResponse } from "../../../types/transactions";
...
const RequestRestaurantRole = () => {
  ...
  const {
    ...
    handleSubmit,
    reset,
  } = useForm<RestaurantRequestFormPayload>();

  const patchRestaurantRequest = useMutation<
    AxiosResponse<RestaurantRolePatchResponse>,
    Error,
    RestaurantRequestPatchData
  >({
    mutationFn: async (patchData) => {
      const restaurantRequestRes =
        await axiosSecure.patch<RestaurantRolePatchResponse>(
  ...
  const onSubmit = async (data: RestaurantRequestFormPayload) => {
    // image file
    ...
    try {
      ...
      let formattedContact = data?.organization_contact
        ? data.organization_contact.trim()
        : "";
      if (formattedContact.startsWith("0")) {
        formattedContact = "+880" + formattedContact.slice(1);
      }
      // patch user in database
      const patchData: RestaurantRequestPatchData = {
      ...
      const res = await patchRestaurantRequest.mutateAsync(patchData);
      if (res?.data?.userUpdate?.modifiedCount > 0) {
        (Swal.fire({
          icon: "success",
          title: "Restaurant role request submitted successfully.",
      ...
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <input
            ...
            {...register("organization_tagline", {
              required: "Restaurant tag line is required.",
            })}
          />
        ...
        <div>
          ...
          <input
            ...
            {...register("mission", {
              required: "Restaurant mission is required.",
            })}
          />
          {errors?.mission && (
        ...
        </div>
        {
          <div className="my-4 space-y-1 text-sm text-teal-700 font-medium">
            ...
            {patchRestaurantRequest.isPending && (
        ...
        <div>
          <button
            ...
            disabled={
              patchRestaurantRequest.isPending || restaurantLoading || isPending
            }
          >
            Submit
          </button>
        ...
        {isError && (
          <p className="text-red-600 text-sm">Upload error: {error?.message}</p>
        )}
      </form>
    </section>
  );
