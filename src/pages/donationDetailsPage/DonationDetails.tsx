...
const DonationDetails = () => {
  ...
  const { data: donation, isLoading: donationGetLoading } =
  ...
  const quantity = donation?.quantity ? Number(donation.quantity) : 0;
  const totalFood = `${quantity} ${donation?.unit}${quantity > 1 ? "s" : ""}`;
  const formattedDate = donation
    ? new Date(donation?.pickup_date).toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      })
  ...
  const { data: isExist, isLoading: existLoading } = useQuery<{
    ...
    queryFn: async () => {
      const res = await axiosSecure.get<{ exists: boolean }>(
        `/requests/exist?email=${userEmail}&donationId=${donationId}`,
      );
      return res?.data;
    },
  ...
  const { data: isFavorited, isLoading: favoritedGetLoading } = useQuery<boolean>({
    queryKey: ["isFavorited", donationId, userEmail],
    queryFn: async () => {
      const res = await axiosSecure.get<{ favorited: boolean }>(
        `/favorites/is_favorited?email=${userEmail}&donationId=${donationId}`,
      );
  ...
  const { mutate: addToFavorites, isPending: favoritePending } = useMutation<
    ...
    { favoriteData: FavoritePostData }
  >({
    mutationFn: async ({ favoriteData }) => {
      const res = await axiosSecure.post<FavoritePostResponse>(
        `/favorites?email=${userEmail}`,
        favoriteData,
      );
      return res?.data;
    },
    onSuccess: async () => {
      ...
      await axiosSecure.patch(
        `/donations/add_favorite/${donationId}?email=${userEmail}`,
      );
  ...
  const handleFavorites = () => {
    if (donation) {
      const favoriteData: FavoritePostData = {
  ...
  return (
    <section className="py-10">
      <section className="one  px-5 pt-5 bg-teal-50/50 max-w-4xl mx-auto">
        <h1 className="font-bold text-3xl text-teal-900 italic mb-5">
          Donation Details
        </h1>
        <img
        ...
        />
        <h2 className="text-2xl font-semibold italic text-teal-800 my-4">
