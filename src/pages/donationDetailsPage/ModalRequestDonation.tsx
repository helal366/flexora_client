...
import { BackendRequestPayload, RequestFormInputs } from '../../types/requests';
import { AxiosError } from 'axios';

interface ModalRequestDonationProps {
  isOpen: boolean;
  handleClose: () => void;
  donation: FoodDonation | undefined;
  setIsDisabled: React.Dispatch<React.SetStateAction<boolean>>;
}
interface RequestPostResponse {
  success: boolean;
  message?: string;
  insertedId?: string;
}
const ModalRequestDonation = ({
  isOpen,
  handleClose,
  donation,
  setIsDisabled,
}: ModalRequestDonationProps) => {
  ...
  >({
    mutationFn: async (requestData) => {
      const res = await axiosSecure.post<RequestPostResponse>(
          ...
          >
            ...
            <div>
              ...
              <input
                type="date"
                className="input input-bordered w-full"
                {...register("preferred_pickup_date", {
            ...
            {errors?.preferred_pickup_date && (
              <p className="text-red-600 text-xs mt-1">
                {errors.preferred_pickup_date?.message}
              </p>
            )}
            ...
            <div>
              ...
              <input
                type="time"
                className="input input-bordered w-full"
                {...register("preferred_pickup_time", {
                  required: "Preferred Pickup time is required.",
                })}
              />
            </div>
            {errors?.preferred_pickup_time && (
              <p className="text-red-600 text-xs mt-1">
                {errors.preferred_pickup_time?.message}
              </p>
            )}
            {/* buttons */}
